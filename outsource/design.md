...
- [ ] [Определить опорный размер](#определить-опорный-размер)
...
В то же время очень рекомендуем не отступать от опорных размеров (как правило всегда можно задизайнить так, что бы
все было удобно).
...
Для блоков очень круто предусматривать не размер как таковой, а его отношение с контентом. То есть, например, у нас
есть блок и в нем есть текстовый контент. Будет очень круто если вы опишите размер шрифта для этого контента и какие
у него отступы, максимальную и минимальную ширину (если это применимо) вместо того, что бы просто задать некоторый фикс
размер.
...
Многие из этих проблем можно исключить путём использования 8pt сетки и [опорного размера](#определить-опорный-размер). Отличная статья на эту тему: [8pt сетка](https://designpub.ru/https-medium-com-simberev-8pt-grid-for-web-f05fccc956ab)
...
В самом идеальном случае все иконки должны быть вписаны в одинаковый контейнер, например,
64px на 64px с необходимыми в каждом конкретном случае сейфсозанми - это максимально упростит их использование и
масштабирование.

Общая просьба при создании иконок - старайтесь до последнего использовать самые простые функции рисования: квадраты,
круги, линии и так далее. Везде где это возможно не используйте эффекты и разного рода наложения. Это поможет уменьшить
вес иконки и очень поможет в случае, если ваши иконки нужно будет анимировать.
